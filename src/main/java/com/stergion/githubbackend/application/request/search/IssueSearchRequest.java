...
public final class IssueSearchRequest implements ContributionSearchRequest {
    private static final ObjectMapper MAPPER = JsonObjectMapper.create();
    // time
    public final Map<IssueTimeField, RangeValue<LocalDateTime>> timeFilters =
            new HashMap<>();
    // range
    public final Map<IssueRangeField, RangeValue<Integer>> rangeFilters =
            new HashMap<>();
    @Min(0)
    public int page;
    @Min(20)
    public int size;
    public IssueField sortBy = IssueField.CREATED_AT;
    ...
    public void setSortBy(String sortBy) {
        this.sortBy = IssueField.valueOf(sortBy.trim().toUpperCase());
    }

    @JsonSetter("state")
    public void setState(String state) {
        this.state = IssueState.valueOf(state.trim().toUpperCase());
    }


    public void addTimeFilter(String field, RangeValue<LocalDateTime> value) {
        for (var e : IssueTimeField.values()) {
    ...
    public void addRangeFilter(String field, RangeValue<Integer> value) {
        for (var e : IssueRangeField.values()) {
            if (e.toString().equalsIgnoreCase(field)
                    || e.getField().equalsIgnoreCase(field)) {
    ...
    @Override
    public Map<IssueTimeField, RangeValue<LocalDateTime>> timeFilters() {
        return this.timeFilters;
    }

    @Override
    public Map<IssueRangeField, RangeValue<Integer>> rangeFilters() {
